...
- 本地 Codex、Claude 与 ZCode（GLM）使用记录的 AI token 用量摘要。
- 实验性充电阈值提醒；暂停或恢复充电只会在用户点击并确认管理员授权后执行。
- 设置窗口：管理外观、字号、模块、刷新间隔、启动行为与权限。

## 安装

1. 从最新 GitHub 发布页下载 `NotchFlow-v0.1.2-macOS.zip`。
...
`v0.1.2` 是当前公开开发者预览版。它用于分发流程验证，当前为本地签名，还未使用 Apple Developer ID 公证。

已知限制：

- 部分集成依赖 macOS 权限和第三方应用可用性。
- WeatherKit 授权或定位权限不可用时，天气模块会进入兜底状态。
- 充电阈值提醒仍为实验功能，充电切换需要用户逐次授权，具体表现取决于硬件和 SMC 行为。
